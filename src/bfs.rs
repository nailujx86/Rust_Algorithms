...
/// tree.add_link(link1);
/// 
/// let result = bfs_search_node(tree, node1.id, node2.id).unwrap();
...
mod discover_test {
    ...
    fn test_discover_no_start_element() {
        ...
        tree.add_node(node1);
        assert_eq!(bfs_search_node(tree, 2, 1).is_none(), true);
    }
    ...
    fn test_discover_no_target_element() {
        ...
        let node1 = Node::new("Node 1");
        tree.add_node(node1);
        assert_eq!(bfs_search_node(tree, 1, 2).is_none(), true);
    }
    ...
    fn test_discover_element_to_self() {
        let tree = Tree::new();
        let result = bfs_search_node(tree, 1, 1).unwrap();
    ...
    fn test_discover_multiple_elements() {
        ...
        tree.add_link(link5);
        tree.add_link(link6);
        let result = bfs_search_node(tree, node1.id, node7.id).unwrap();
